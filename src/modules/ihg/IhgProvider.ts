...
import { ihgSelectors } from './ihg.selectors.js';
...
export class IhgProvider implements HotelPriceProvider {
  ...
  private async runQuery(input: PriceQuery): Promise<HotelPriceResult> {
    ...
    try {
      await page.goto(ihgSelectors.homeUrl, { waitUntil: 'domcontentloaded' });
      ...
      if (!match) {
        ...
        return unavailableResult(this.name, input, 'hotel_not_found', 'No matching IHG hotel found', page.url(), artifact);
      }

      const candidate = await this.extractRate(match.card);
      if (!candidate.price) {
        const artifact = await saveFailureArtifact(this.name, page, 'no-availability');
        return unavailableResult(this.name, input, 'no_availability', 'No rate found on matched IHG hotel', page.url(), artifact);
      }
  ...
  private async fillSearch(page: Page, input: PriceQuery): Promise<void> {
    const destinationInput = await firstVisible(page, ihgSelectors.destinationInput);
    ...
    const searchButton = await firstVisible(page, ihgSelectors.searchButton);
    await Promise.all([
  ...
  private async tryFillDate(page: Page, field: 'checkIn' | 'checkOut', value: string): Promise<void> {
    const selector = field === 'checkIn'
      ? 'input[name*="checkIn"], input[aria-label*="Check-in"], input[placeholder*="Check-in"]'
      : 'input[name*="checkOut"], input[aria-label*="Check-out"], input[placeholder*="Check-out"]';
  ...
  private async findBestHotel(page: Page, hotelName: string): Promise<{ card: Locator; name: string; confidence: number } | undefined> {
    const cards = await firstLocatorWithItems(page, ihgSelectors.hotelCards);
    const count = Math.min(await cards.count(), 20);
    let best: { card: Locator; name: string; confidence: number } | undefined;

    for (let index = 0; index < count; index += 1) {
      const card = cards.nth(index);
      const name = await textFromFirst(card, ihgSelectors.hotelName);
  ...
  private async extractRate(card: Locator): Promise<RateCandidate> {
    const priceText = await textFromFirst(card, ihgSelectors.priceText);
